    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/services/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(services?.services?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (services > 0) {
      return services;
    } else if (services === 0 && isFiltered) {
      return [];
    } else {
      return services?.services?.services?.slice(0, rowsPerPage);
    }
  };
