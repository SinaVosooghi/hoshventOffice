...
const ItemList = () => {
  ...
  const [getItems, { data: brands }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(
      Math.ceil(brands?.brands?.count / rowsPerPage)
    );
  ...
  const dataToRender = () => {
    ...
    if (brands > 0) {
      return brands;
    } else if (brands === 0 && isFiltered) {
      return [];
    } else {
      return brands?.brands?.brands?.slice(0, rowsPerPage);
    }
  };
