    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to="/apps/blogs/add" color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: blogs }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(blogs?.blogs?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (blogs > 0) {
      return blogs;
    } else if (blogs === 0 && isFiltered) {
      return [];
    } else {
      return blogs?.blogs?.blogs?.slice(0, rowsPerPage);
    }
  };
