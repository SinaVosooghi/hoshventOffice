    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button tag={Link} to="/apps/courses/add" color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: courses }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(courses?.courses?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (courses > 0) {
      return courses;
    } else if (courses === 0 && isFiltered) {
      return [];
    } else {
      return courses?.courses?.courses?.slice(0, rowsPerPage);
    }
  };
