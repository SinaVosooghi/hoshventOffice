    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/departments/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: departments }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(
      Math.ceil(departments?.departments?.count / rowsPerPage)
    );
  ...
  const dataToRender = () => {
    ...
    if (departments > 0) {
      return departments;
    } else if (departments === 0 && isFiltered) {
      return [];
    } else {
      return departments?.departments?.departments?.slice(0, rowsPerPage);
    }
  };
