...
const ItemList = () => {
  ...
  const [getItems, { data: reviews }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(reviews?.reviews?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (reviews > 0) {
      return reviews;
    } else if (reviews === 0 && isFiltered) {
      return [];
    } else {
      return reviews?.reviews?.reviews?.slice(0, rowsPerPage);
    }
  };
