...
const ItemList = () => {
  ...
  const [getItems, { data: payments }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(payments?.payments?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (payments > 0) {
      return payments;
    } else if (payments === 0 && isFiltered) {
      return [];
    } else {
      return payments?.payments?.payments?.slice(0, rowsPerPage);
    }
  };
