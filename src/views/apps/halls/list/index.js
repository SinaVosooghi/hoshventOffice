    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/halls/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: halls }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(halls?.halls?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (halls > 0) {
      return halls;
    } else if (halls === 0 && isFiltered) {
      return [];
    } else {
      return halls?.halls?.halls?.slice(0, rowsPerPage);
    }
  };
