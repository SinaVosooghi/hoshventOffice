    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/plans/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: plans }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(plans?.plans?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (plans > 0) {
      return plans;
    } else if (plans === 0 && isFiltered) {
      return [];
    } else {
      return plans?.plans?.plans?.slice(0, rowsPerPage);
    }
  };
