    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/sliders/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
        ...
        >
          <div className="d-flex align-items-center ms-2 x">
...
const ItemList = () => {
  ...
  const [getItems, { data: sliders }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(sliders?.sliders?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (sliders > 0) {
      return sliders;
    } else if (sliders === 0 && isFiltered) {
      return [];
    } else {
      return sliders?.sliders?.sliders?.slice(0, rowsPerPage);
    }
  };
