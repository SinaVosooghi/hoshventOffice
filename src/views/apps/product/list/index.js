    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button tag={Link} to="/apps/products/add" color="primary">
            {t("Add new product")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: products }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(products?.products?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (products > 0) {
      return products;
    } else if (products === 0 && isFiltered) {
      return [];
    } else {
      return products?.products?.products?.slice(0, rowsPerPage);
    }
  };
