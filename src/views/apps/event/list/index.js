    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to="/apps/events/add" color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: events }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(events?.events?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (events > 0) {
      return events;
    } else if (events === 0 && isFiltered) {
      return [];
    } else {
      return events?.events?.events?.slice(0, rowsPerPage);
    }
  };
