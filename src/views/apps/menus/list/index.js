...
const ItemList = () => {
  ...
  const [getItems, { data: menus }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(menus?.menus?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (menus > 0) {
      return menus;
    } else if (menus === 0 && isFiltered) {
      return [];
    } else {
      return menus?.menus?.menus?.slice(0, rowsPerPage);
    }
  };
