...
export const columns = [
  {
    ...
    cell: (row) => (
      <Link
        to={`/apps/invoice/preview/${row?.invoice?.id}`}
      >{`#${row?.invoice?.id}`}</Link>
    ),
  },
  {
    ...
    cell: (row) => {
      return (
        <Link to={`/apps/invoice/preview/${row?.invoice?.id}`}>
  ...
  },
  {
    ...
    cell: (row) => <span>{thousandSeperator(row?.invoice?.total) || 0}</span>,
  },
  {
    sortable: true,
    name: t("Issued Date"),
    ...
    cell: (row) => (
      <span className="text-capitalize">
        <MultiLingualDatePicker date={row?.invoice?.issuedate} />
      </span>
    ),
  },
  {
    ...
    cell: (row) => (
      <span className="text-capitalize">
        <MultiLingualDatePicker date={row?.invoice?.duedate} />
      </span>
    ),
  },
  {
        ...
        <div className="column-action d-flex align-items-center">
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu end>
              ...
              <DropdownItem
                tag={Link}
                to={`/apps/invoice/edit/${row?.invoice.id}`}
                className="w-100"
              >
              ...
              <DropdownItem
                tag={Link}
                to={`/apps/invoice/print/${row?.invoice.id}`}
                className="w-100"
              >
