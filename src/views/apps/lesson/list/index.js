    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        <Col lg="6" className="d-flex align-items-center px-0 px-lg-1">
          ...
          <Button tag={Link} to="/apps/lessons/add" color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: lessons }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(lessons?.lessons?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (lessons > 0) {
      return lessons;
    } else if (lessons === 0 && isFiltered) {
      return [];
    } else {
      return lessons?.lessons?.lessons?.slice(0, rowsPerPage);
    }
  };
