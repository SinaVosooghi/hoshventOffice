...
export const columns = [
  {
    ...
    cell: (row) => {
      ...
      const user = row.user
        ...
        : "-";
      return (
        <Link to={`/apps/lessons/edit/${row.id}`}>
          <div className="d-flex justify-content-left align-items-center">
            {renderClient(row)}
            <div className="d-flex flex-column">
              <h6 className="user-name text-truncate mb-0">{name}</h6>
              <small className="text-truncate text-muted mb-0">{user}</small>
            </div>
          </div>
        </Link>
      );
  ...
  },
  {
    ...
    minWidth: "150px",
    sortField: "total",
    cell: (row) => <span>{row.course ? row.course?.title : "-"}</span>,
  },
  {
    name: t("Type"),
    sortable: true,
  ...
  },
  {
    ...
    cell: (row) => (
      <span className="text-capitalize">
        <Badge
          className="text-capitalize"
          color={!row.public ? "light-danger" : "light-success"}
          pill
        >
          {row.public ? t("Active") : t("Deactive")}
        </Badge>
      </span>
    ),
  },
