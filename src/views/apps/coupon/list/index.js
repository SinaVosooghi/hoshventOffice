    ...
    <div className="invoice-list-table-header w-100 py-2">
      <Row>
        ...
        >
          ...
          <Button tag={Link} to={`/apps/coupons/${type}/add`} color="primary">
            {t("Add Record")}
          </Button>
        </Col>
        <Col
...
const ItemList = () => {
  ...
  const [getItems, { data: coupons }] = useLazyQuery(GET_ITEMS_QUERY, {
  ...
  const CustomPagination = () => {
    const count = Number(Math.ceil(coupons?.coupons?.count / rowsPerPage));
  ...
  const dataToRender = () => {
    ...
    if (coupons > 0) {
      return coupons;
    } else if (coupons === 0 && isFiltered) {
      return [];
    } else {
      return coupons?.coupons?.coupons?.slice(0, rowsPerPage);
    }
  };
